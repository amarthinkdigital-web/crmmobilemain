...
class EventsMeetingsScreen extends StatefulWidget {
  const EventsMeetingsScreen({super.key});

  @override
  State<EventsMeetingsScreen> createState() => _EventsMeetingsScreenState();
}

class _EventsMeetingsScreenState extends State<EventsMeetingsScreen> {
  ...
  Future<void> _fetchEvents() async {
    ...
    final res = await ApiService.getAdminEvents();
    if (mounted) {
      if (res['error'] == false) {
        setState(() {
          _events = res['data'] ?? [];
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  _buildInputField('Event Title', 'Enter title', titleController, Icons.title_rounded),
                    ...
                    children: [
                      Expanded(
                        child: _buildInputField(
                          'Start Date', 'Select start date', startController,
                      ...
                      Expanded(
                        child: _buildInputField(
                          'End Date', 'Select end date', endController,
                    ...
                    child: ElevatedButton(
                      onPressed: isSaving ? null : () async {
                        ...
                        final body = {
                          ...
                          'name': titleController.text, // Fallback for some backend models
                        ...
                        try {
                          ...
                          if (isEdit) {
                            res = await ApiService.updateAdminEvent(event['id'], body);
                          } else {
                            res = await ApiService.createAdminEvent(body);
                          }
  ...
  Future<void> _deleteEvent(dynamic id) async {
    ...
    if (confirm == true) {
      final res = await ApiService.deleteAdminEvent(id);
      if (mounted) {
        if (res['error'] == false) {
          _fetchEvents();
          ScaffoldMessenger.of(context).showSnackBar(
  ...
  void _showEventDetails(dynamic event) {
    ApiService.markAdminEventViewed(event['id']);
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
  ...
  Widget _buildHeader() {
    return Column(
      ...
      children: [
        ...
        Text(
          'Schedule and manage company-wide events and team meetings',
            ...
            child: SingleChildScrollView(
              ...
              child: DataTable(
                ...
                rows: _events.asMap().entries.map((entry) {
                  ...
                  return DataRow(
                    cells: [
                      DataCell(Text((idx + 1).toString(), style: GoogleFonts.inter(fontSize: 13))),
                      DataCell(Text(
                      ...
                      DataCell(Text(_formatDate(event), style: GoogleFonts.inter(fontSize: 12, color: AppColors.grey600))),
                      DataCell(Text(_formatEndDate(event), style: GoogleFonts.inter(fontSize: 12, color: AppColors.grey600))),
                      DataCell(_buildVisibilityBadge(_getVisibility(event))),
                      DataCell(Row(
                        children: [
                          _actionIcon(Icons.visibility_outlined, AppColors.info, 'View', () => _showEventDetails(event)),
                          const SizedBox(width: 6),
                          _actionIcon(Icons.edit_outlined, AppColors.goldDark, 'Edit', () => _showEventSheet(event: event)),
                          const SizedBox(width: 6),
                          _actionIcon(Icons.delete_outline_rounded, AppColors.error, 'Delete', () => _deleteEvent(event['id'])),
                        ],
                      )),
                    ],
                  );
