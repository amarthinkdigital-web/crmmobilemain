...
class _EventsScreenState extends State<EventsScreen> {
  ...
  Future<void> _fetchEvents() async {
    ...
    final res = await ApiService.getEmployeeEvents();
    if (mounted) {
      if (res['error'] == false) {
        setState(() {
          _events = res['data'] ?? [];
  ...
  void _showEventDetails(dynamic event) {
    ApiService.markEmployeeEventViewed(event['id']);
    showDialog(
      context: context,
  ...
  Widget _buildHeader() {
    return Column(
      ...
      children: [
        ...
        Text(
          'View and manage company events & meetings',
              ...
              return DataRow(
                cells: [
                  ...
                  DataCell(Builder(builder: (context) {
                    final rawCreator = event['creator_name'] ?? event['created_by_name'] ?? event['user_name'] ?? event['user']?['name'] ?? event['creator']?['name'] ?? event['created_by']?['name'] ?? event['user_fullname'] ?? 'Admin';
                    final creatorName = rawCreator.toString();
                    final creatorId = event['user_id']?.toString() ?? event['created_by_id']?.toString() ?? event['created_by']?.toString();
                  ...
                  DataCell(Text(_formatDate(event), style: GoogleFonts.inter(fontSize: 13, color: AppColors.grey600))),
                  DataCell(Text(_formatEndDate(event), style: GoogleFonts.inter(fontSize: 13, color: AppColors.grey600))),
                  DataCell(
                    Builder(builder: (context) {
                      final visibility = _getVisibility(event);
                  ...
                  DataCell(Builder(builder: (context) {
                    final rawCreator = event['creator_name'] ?? event['created_by_name'] ?? event['user_name'] ?? event['user']?['name'] ?? event['creator']?['name'] ?? event['created_by']?['name'] ?? event['user_fullname'] ?? '';
                    final creatorName = rawCreator.toString();
                    final creatorId = event['user_id']?.toString() ?? event['created_by_id']?.toString() ?? event['created_by']?.toString();
                    ...
                    return Row(
                      children: [
                        _buildActionBtn('View', Icons.visibility_rounded, AppColors.navy, () => _showEventDetails(event)),
                        if (isSelf) ...[
                          const SizedBox(width: 8),
                          _buildActionBtn('Edit', Icons.edit_rounded, AppColors.goldDark, () => _showEventSheet(event: event)),
                          const SizedBox(width: 8),
                          _buildActionBtn('Delete', Icons.delete_outline_rounded, AppColors.error, () => _deleteEvent(event['id'])),
                        ],
                      ],
                    );
                  })),
                    ...
                    child: ElevatedButton(
                      onPressed: isSaving ? null : () async {
                        if (titleController.text.isEmpty || startController.text.isEmpty) {
                          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Title and Start Date are required'), backgroundColor: Colors.orange));
                          return;
                        }
                        ...
                        try {
                          ...
                          if (isEdit) {
                            res = await ApiService.updateEmployeeEvent(event['id'], body);
                          } else {
                            res = await ApiService.createEmployeeEvent(body);
                          }
  ...
  Future<void> _deleteEvent(dynamic id) async {
    ...
    if (confirm == true) {
      final res = await ApiService.deleteEmployeeEvent(id);
      if (mounted) {
        if (res['error'] == false) {
          _fetchEvents();
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Event deleted'), backgroundColor: AppColors.success));
        } else {
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(res['message'] ?? 'Failed to delete'), backgroundColor: AppColors.error));
        }
      }
    }
  }

  Widget _buildInputField(String label, String hint, TextEditingController controller, IconData icon, {bool isReadOnly = false, VoidCallback? onTap, int maxLines = 1}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
